...
const HolidaysNew = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            key: "is_paid",
            render: (is_paid, row) => {
                return <Switch key={row.id} onClick={(e) => paid(e, row)} checked={is_paid ? true : false} disabled={is_paid ? true : false} />
            },
        },
    ];
    ...
    const confirmPaid = async (id) => {
        ...
        try {
            ...
            let response = await Axios.patch(API_URL + `/payroll/payment-plan/${id}/`, data);
    ...
    const getPlan = async () => {
        try {
            let response = await Axios.get(API_URL + `/payroll/payment-plan/?loan__id=${id}`);
