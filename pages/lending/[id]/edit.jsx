...
const HolidaysNew = () => {
  ...
  const onCancel = () => {
    ...
    const approve = async () => {
        ...
        setSending(true);
        try {
            let response = await Axios.post(API_URL+`/payroll/loan/approve_request/`, values);
            Modal.success({
                keyboard: false,
                maskClosable: false,
  ...
  const reject = async () => {
    ...
    try {
        let response = await Axios.post(API_URL+`/payroll/loan/reject_request/`, values);
  ...
  return (
    <MainLayout currentKey="7.1">
      ...
      >
        <Row justify={"center"}>
          <Col span={23}>
            { !loading ? <Lendingform sending={sending} details={details} edit={true} onApprove={approve} onReject={ShowModal} onFinish={onFinish} config={config} /> : null }
          </Col>
        </Row>
      </div>
      <Modal
        ...
        footer={[
            <Button key="back" onClick={rejectCancel} style={{ padding: "0 50px", marginLeft: 15 }} >
              Cancelar
            </Button>,
            <Button key="submit_modal" type="primary" loading={sending} onClick={reject} style={{ padding: "0 50px", marginLeft: 15 }}>
              Aceptar
            </Button>
          ]}
      >
