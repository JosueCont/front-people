...
const index = () => {
    ...
    return (
        ...
        <Global
            ...
            `}
        />
        <MainLayout currentKey={["dashboard"]} nomina >
            <Row justify={'space-between'}>
                <Col md={16} sm={24} style={{paddingRight:'30px !important'}}>
                    <Row justify={'space-between'} gutter={[10,20]} style={{marginTop:20}} >
                        <Col>
                            <Title level={3}>
                                Irvin, te damos la bienvenida a nómina
                            </Title>
                        </Col>
                        <Col>
                            <Button type={'primary'} size="large" className="button_add_card" style={{display:'flex'}}>
                                <PlusCircleOutlined />
                                Agregar indicador
                            </Button>
                        </Col>
                            ...
                            <div position={item.position} key={idx}>
                                {
                                    item.id === 1 || item.id === 6 ?
                                        ...
                                        <Info title={'Total de empresas'} count={5} onOk={() => console.log('ok')} okText={'Ver empresas'} icon={<HomeOutlined/>} /> : 
                                            item.id === 3 || item.id === 8 ? 
                                            <ChartDoughnut title={'Total de colaboradores'} icon={<TeamOutlined/>}  /> : 
                                            <img src="https://menaiortho.com.au/wp-content/uploads/2015/03/banner-placeholder.jpg" style={{width:'100%'}} />
                                }
                            </div>
                        )}
