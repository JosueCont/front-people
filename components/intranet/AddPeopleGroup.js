...
const AddPeopleGroup = (props) => {
    ...
    const onFinish = (values) => {
        ...
        axios.patch(API_URL + `/intranet/group/${props.group.id}/`, data)
            .then((res) => {
                props.setVisible()
            })
            .catch((e) => {
    ...
    return (<>
        <Layout>
            <Modal
                maskClosable={false}
                title={props.group.user.length>0 ? "Editar lista de personas del "+ props.group.name : "Agregar personas al " + props.group.name}
                centered
