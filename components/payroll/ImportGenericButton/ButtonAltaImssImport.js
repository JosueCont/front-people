...
const ButtonAltaImssImport=({person, regPatronal=null,node, payrollPerson, personsList,...props})=>{
    const [loading, setLoading] = useState(false)
    const [showModal, setShowModal] = useState(false)
    const [form] = Form.useForm();
    const user = useSelector(state => state.userStore.user);
    const regPatronals = useSelector(state => state.catalogStore.cat_patronal_registration);
    ...
    const openModal=(type=MOVEMENTS_TYPE.UPDATE_SALARY)=>{
        ...
        console.log(personsList)
            ...
            setCurrentMovement({
                ...
                description: `Esta sección te permite generar alta de persona con los datos mínimos de IMSS. Estará ligado al registro patronal seleccionado ${regPatronalSelected && regPatronalSelected.code}. `
            })
            setShowModal(true)
    }
    ...
    return (
        <>
            <Button disabled={!regPatronal} onClick={() => openModal(MOVEMENTS_TYPE.IMSS_REGISTER)} style={menuDropDownStyle}>
                Generar alta Imss
            </Button>
            ...
            >
                <Row>
                    <Col span={24}>
                        <p><b>Reg.Patronal: </b>{regPatronalSelected && regPatronalSelected.code}</p>
