...
const AssignAssessments = ({ itemGroup, ...props }) => {
  ...
  const getListAssigned = () => {
    let list = listSurveys.results ? listSurveys.results : listSurveys;
  ...
  const getListCopy = () => {
    let list = copyList.results ? copyList.results : copyList;
    return list;
  };
  ...
  const onFinish = (values) => {
    const data = createData({
      assessments: surveysKeys,
      groups_assessment: groupsKeys,
    });
      ...
      >
        <Row gutter={[8, 16]}>
          ...
          <Col span={24}>
            <Table
              rowKey={"id"}
              columns={isIndividual ? columnsInvidual : columnsGroup}
