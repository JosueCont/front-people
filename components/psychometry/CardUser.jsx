...
const CardUser = ({ user_profile, user_assessments, loading, ...props }) => {
  ...
  const setValues = () => {
    if (Object.keys(user_profile).length > 0) {
      setUser({
        fullName: `${user_profile.first_name} ${user_profile.flast_name} ${
          user_profile.mlast_name ? user_profile.mlast_name : ""
        }`,
  ...
  const calculateIndicators = () => {
    ...
    user_assessments.map((item) => {
      ...
      if (isArray ? item.apply[0] : item.apply) {
        if (
          isArray
            ? item.apply[0].progress == 100 || item.apply[0].status == 2
            : item.apply.progress == 100 || item.apply.status == 2
        ) {
    ...
    <Row gutter={[24, 24]} align={"middle"}>
      <Col xs={24} sm={24} md={24} lg={14} xl={16}>
        <Card
          loading={loading}
          bordered={false}
          style={{ background: "#814cf2", borderRadius: "12px" }}
        >
          <Row gutter={[16, 24]} align="middle">
            <Col xs={24} sm={16} md={18} lg={16} xl={19}>
              <ContentStart gap={"16px"}>
                <Avatar shape={"circle"} size={62} src={user.photo} />
